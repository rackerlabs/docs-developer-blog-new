...
muffet:
	...
	@$(CURDIR)/output/muffet/bin/muffet --exclude \#\!\(forum\|msg\)\/developer-docs-blog-\(dev\|users\)\/? --exclude https?:\/\/localhost:20001\/? --exclude \/documentation\/developer-api http://localhost:1313/

# Targets used to build and use the Hugo+Asciidoctor docker image

.PHONY: hugo-docker
hugo-docker:
	${CONTAINER_RUNTIME} build --build-arg HUGO_VERSION=${HUGO_VERSION} -t developer-docs-blog/hugo:${HUGO_VERSION} hugo

.PHONY: hugo-serve
hugo-serve: hugo-docker
	@mkdir -p ./resources/_gen/images && mkdir -p ./resources/_gen/assets
	@${CONTAINER_RUNTIME} run -t -i --sig-proxy=true --rm --mount type=bind,src=$(shell pwd),dst=/site -w /site -p 1313:1313 developer-docs-blog/hugo:${HUGO_VERSION} /hugo serve --baseURL "http://localhost:1313/" --bind 0.0.0.0 --disableFastRender
	@rm -rf ./resources
